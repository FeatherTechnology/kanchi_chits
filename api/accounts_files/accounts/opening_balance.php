...
$s_cr_h_qry = $pdo->query("SELECT SUM(settle_cash) AS settlr_cr_amnt FROM settlement_info WHERE settle_type = 1 AND  DATE(created_on) <= ('$current_date' - INTERVAL 1 DAY)"); //Hand Cash
if ($s_cr_h_qry->rowCount() > 0) {
    $s_cr_h = $s_cr_h_qry->fetch()['settlr_cr_amnt'];
} else {
    $s_cr_h = 0;
}
$s_cr_b_qry = $pdo->query("SELECT COALESCE(SUM(cheque_val) + SUM(transaction_val),0) AS settlr_br_amnt FROM settlement_info WHERE settle_type >= 2 AND DATE(created_on) <= ('$current_date' - INTERVAL 1 DAY)"); //Hand Cash
...
$ot_dr_h_qry = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM other_transaction WHERE coll_mode = 1 AND type = 2 AND trans_cat !=7 AND DATE(created_on) <= ('$current_date' - INTERVAL 1 DAY) "); //Hand Cash //debit
...
$ot_dr_b_qry = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM other_transaction WHERE coll_mode = 2 AND type = 2 AND trans_cat !=7 AND DATE(created_on) <= ('$current_date' - INTERVAL 1 DAY) "); //Bank Cash //debit
